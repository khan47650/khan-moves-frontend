...
import { FaCouch, FaBed, FaChair, FaTv, FaTshirt } from 'react-icons/fa';
import { MdTableRestaurant, MdMicrowave } from 'react-icons/md';
...
const FURNITURE_INVENTORY = {
    Sofas: {
        icon: FaCouch, items: [
            { name: 'Two Seater Sofa', volume: 800 },
            { name: 'Three Seater Sofa', volume: 1100 },
            { name: 'Four Seater Sofa', volume: 1400 },
            { name: 'L-Shaped Sofa', volume: 1600 },
            { name: 'Corner Sofa Bed', volume: 1700 },
            { name: 'Recliner Sofa', volume: 900 },
        ]
    },
    Wardrobes: {
        icon: FaTshirt, items: [
            { name: 'Single Wardrobe', volume: 600 },
            { name: 'Double Wardrobe', volume: 1000 },
            { name: 'Triple Wardrobe', volume: 1400 },
            { name: 'Sliding Wardrobe', volume: 1200 },
        ]
    },
    'Beds & Mattresses': {
        icon: FaBed, items: [
            { name: 'Single Bed & Mattress', volume: 700 },
            { name: 'Double Bed & Mattress', volume: 1000 },
            { name: 'King Size Bed & Mattress', volume: 1300 },
            { name: 'Super King Bed', volume: 1500 },
            { name: 'Bunk Bed', volume: 1100 },
        ]
    },
    Tables: {
        icon: MdTableRestaurant, items: [
            { name: 'Dining Table', volume: 500 },
            { name: 'Coffee Table', volume: 200 },
            { name: 'Side Table', volume: 100 },
            { name: 'Desk', volume: 400 },
        ]
    },
    Chairs: {
        icon: FaChair, items: [
            { name: 'Dining Chair', volume: 100 },
            { name: 'Armchair', volume: 400 },
            { name: 'Office Chair', volume: 200 },
            { name: 'Stool', volume: 50 },
        ]
    },
    Televisions: {
        icon: FaTv, items: [
            { name: 'TV up to 32"', volume: 100 },
            { name: 'TV 32-50"', volume: 200 },
            { name: 'TV 50-65"', volume: 300 },
            { name: 'TV 65"+', volume: 400 },
        ]
    },
    Appliances: {
        icon: MdMicrowave, items: [
            { name: 'Fridge / Freezer', volume: 800 },
            { name: 'Washing Machine', volume: 400 },
            { name: 'Dryer', volume: 400 },
            { name: 'Dishwasher', volume: 400 },
            { name: 'Microwave', volume: 100 },
            { name: 'Oven', volume: 500 },
        ]
    },
    'Boxes & Bags': {
        icon: BsBoxSeam, items: [
            ...
            { name: 'Suitcase', volume: 150 },
            { name: 'Duffle Bag', volume: 100 },
        ]
    },
};
...
export default function StepItemSelectionFurniture({ items, onChange, error, pickup, delivery }) {
    ...
    const searchResults = searchQuery
        ? Object.values(FURNITURE_INVENTORY).flatMap(c => c.items).filter(it => it.name.toLowerCase().includes(searchQuery.toLowerCase()))
        : [];
                ...
                <div className="lg:col-span-2">
                    <div className="bg-white rounded-2xl p-4 md:p-5" style={{ boxShadow: '0 2px 12px rgba(0,0,0,0.04)' }}>
                        <div className="relative mb-3">
                            ...
                            <input type="text" placeholder="Search for your item(s) e.g. Sofa" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)}
                        ...
                        {searchQuery ? (
                            ...
                            <>
                                <p className="text-xs text-gray-500 mb-2">Or quickly add from our list of popular items below:</p>
            ...
            {showCustomModal && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" onClick={() => setShowCustomModal(false)}>
                    <div className="bg-white rounded-2xl p-6 w-full max-w-md" onClick={(e) => e.stopPropagation()}>
                        ...
                        <p className="text-xs text-gray-500 mb-4">Tell us what you'd like to add to your move.</p>
                        <input type="text" autoFocus placeholder="e.g., Piano, Treadmill, Antique Vase" value={customName}
